...
export const TUNING_FIELDS: readonly TuningFieldDef[] = [
  ...
  },
  {
    ...
    tip: 'How far a predator detects rabbits — the strongest single lever on whether the prey population survives. Below rabbit sense means prey usually spots the hunter first.',
    section: 'predator',
    tier: 'immediate',
  },
  {
    key: 'predatorHuntThreshold',
    label: 'hunts above hunger',
    min: 0,
    max: 0.6,
    step: 0.01,
    format: (v) => v.toFixed(2),
    tip: 'A predator ignores prey until this hungry. Set to 0 and a permanently-full predator keeps killing, cropping the prey past what it needs and starving itself in the crash.',
    section: 'predator',
    tier: 'terminal',
  },
  {
    key: 'predatorPatrolFactor',
    label: 'patrol speed',
    min: 0.2,
    max: 1,
    step: 0.05,
    format: (v) => `${Math.round(v * 100)}% of speed`,
    tip: 'Fraction of full speed used when not chasing. Sprinting is what the speed² cost makes expensive — at 100% a predator cannot balance its energy budget however well it hunts.',
    section: 'predator',
    tier: 'cosmetic',
  },
  {
  ...
  },
  {
    ...
    max: 300,
    step: 10,
    tip: 'Energy a predator recovers per kill — its only refill path, the counterpart to a rabbit eating a plant.',
    section: 'shared',
    tier: 'cosmetic',
  },
  {
    key: 'returnSafetyMargin',
    label: 'return safety margin',
    min: 1,
    max: 4,
    step: 0.1,
    format: (v) => `${v.toFixed(1)}×`,
    tip: 'How early a creature turns for home. Urgency is computed from the straight-line distance, but real paths detour around lakes — too low and creatures are caught out at nightfall and die of exposure.',
    section: 'shared',
    tier: 'immediate',
  },
